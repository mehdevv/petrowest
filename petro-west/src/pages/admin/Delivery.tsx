...
export default function DeliveryPrices() {
  ...
  const saveMut = useSaveDeliveryPrices({
    mutation: {
      onSuccess: () => {
        ...
        toast({ title: "Delivery prices saved successfully." });
      }
    }
  });
  ...
  const handleReset = () => {
    if(confirm("Reset all wilayas to 500 DA?")) {
  ...
  return (
    <AdminLayout>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          <h1 className="font-display text-4xl text-primary mb-1">Delivery Prices</h1>
          <p className="text-muted-foreground">Set Cash on Delivery pricing for all 58 wilayas.</p>
        </div>
        <div className="flex gap-3">
          ...
          <Button variant="secondary" onClick={handleSaveAll} disabled={saveMut.isPending} className="hover-elevate">
            <Save className="w-4 h-4 mr-2" /> {saveMut.isPending ? "Saving..." : "Save All Prices"}
          </Button>
        </div>
      </div>
        ...
        <Table>
          <TableHeader className="bg-[#001D3D] hover:bg-[#001D3D]">
            <TableRow>
              ...
              <TableHead className="text-white font-bold tracking-wider">Wilaya Name</TableHead>
              <TableHead className="text-right text-white font-bold tracking-wider">Price (DA)</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {isLoading && <TableRow><TableCell colSpan={3} className="text-center py-8">Loading wilayas...</TableCell></TableRow>}
            {pricesData?.map((item) => (
