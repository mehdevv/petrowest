  ...
  const toggleMutation = useToggleProductStock({
    mutation: {
      onSuccess: () => {
        ...
        toast({ title: "Statut du stock mis à jour." });
      }
    }
  });
  ...
  return (
    <AdminLayout>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          <h1 className="font-display text-4xl text-primary mb-1">Produits</h1>
        ...
        <Button asChild variant="secondary" className="hover-elevate">
          <Link href="/admin/products/new"><PlusCircle className="w-4 h-4 mr-2"/> Ajouter un Nouveau Produit</Link>
        </Button>
      </div>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(o) => !o && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Cela supprimera définitivement le produit. Cette action est irréversible.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
