...
export default function Categories() {
  ...
  const deleteMut = useDeleteCategory({
    mutation: {
      ...
      onError: () => {
        toast({ title: "Impossible de supprimer une catégorie avec des produits assignés.", variant: "destructive" });
      }
    }
  });
  ...
  return (
    <AdminLayout>
      <div className="mb-8">
        <h1 className="font-display text-4xl text-primary mb-1">Catégories</h1>
      ...
      <div className="max-w-4xl">
        ...
        <form onSubmit={handleCreate} className="bg-white p-6 rounded-xl border mb-8">
          <label className="text-sm font-bold text-primary mb-4 block uppercase tracking-wider">Ajouter une Nouvelle Catégorie</label>
            ...
            <div className="flex items-center gap-2">
              ...
              {newCatImage ? (
                <div className="relative">
                  <img src={newCatImage} alt="Aperçu" className="w-12 h-12 rounded-lg object-cover border" />
                  <button type="button" onClick={() => setNewCatImage(null)} className="absolute -top-1 -right-1 bg-red-500 text-white rounded-full p-0.5">
                    <X className="w-3 h-3" />
                  </button>
                </div>
              ) : (
                      ...
                      <div className="relative">
                        ...
                        {(editImage || cat.image) ? (
                          <div className="relative inline-block">
                            <img src={editImage || cat.image} alt="" className="w-10 h-10 rounded-lg object-cover border cursor-pointer" onClick={() => editImageRef.current?.click()} />
                            <button type="button" onClick={() => setEditImage("")} className="absolute -top-1 -right-1 bg-red-500 text-white rounded-full p-0.5">
                              <X className="w-3 h-3" />
                            </button>
                          </div>
                        ) : (
                          <button type="button" onClick={() => editImageRef.current?.click()} className="w-10 h-10 rounded-lg border-2 border-dashed border-gray-300 flex items-center justify-center bg-gray-50" disabled={uploadingEdit}>
                    ...
                    {editingId === cat.id ? (
                      <Input 
                        ...
                        onKeyDown={(e) => {
                          ...
                          if (e.key === "Escape") { setEditingId(null); setEditImage(null); }
                        }}
                      />
                    ) : (
                  ...
                  <TableCell className="text-right">
                    {editingId === cat.id ? (
                      <div className="flex justify-end gap-2">
                        <Button size="icon" variant="ghost" onClick={() => updateMut.mutate({ id: cat.id, data: { name: editName, image: editImage !== null ? editImage : cat.image }})} disabled={updateMut.isPending}>
                      ...
                      <div className="flex justify-end gap-2">
                        <Button size="icon" variant="ghost" onClick={() => { setEditingId(cat.id); setEditName(cat.name); setEditImage(cat.image || null); }}>
